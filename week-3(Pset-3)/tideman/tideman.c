...
#include <ctype.h>
#include <stdio.h>
#include <string.h> // compare strings, strcmp()
...
bool lock_check(int start, int current);
...
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        if (lock_check(loser, winner)) // check if lock creates a cycle
        {
...
bool lock_check(int start, int current)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[i][current] && lock_check(start, i))
        {
            return true;
        }
    }
